...
Per strutturare questo applicativo ho scelto di utilizzare l'architettura "Clean Architecture" in quanto non sapevo dove mettere la parte di accesso a DB nel pattern MVC, in quanto non era parte del model da un punto di vista semantico ma rendere dipendente il model sul controller non sembrava nemmeno la soluzione migliore. Inoltre introduce delle ottime regole per il decoupling, più specifiche (almeno per quello che ne ho capito io) di quelle dell'architetura MVC. La clean architecture rimane comunque una evoluzione, a mio avviso, di MVC, qundi la scelta non si è orientata verso una architettura diversa, quanto una sua versione più formale.
